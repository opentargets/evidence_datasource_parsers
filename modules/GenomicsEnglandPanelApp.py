...
class PanelApp_evidence_generator():
    ...
    def check_xref(phenotype, code, mappings_dict, codes_dict):
        '''
        Among the Fuzzy results of a phenotype query, it checks if the term and the respective code points to the same label
        '''
    ...
    def get_evidence_string(self, row):
        ...
        evidence_field = {
                    ...
                    'allelic_requirement' : self.mode_of_inheritance, #TODO: Quitarlo si None
                    'original_disease_array' : self.phenotypes_array
                    }
    ...
    def write_evidence_strings(self, dataframe, mappings_dict):
        ...
        dataframe = dataframe.sample(frac=0.001)
        ...
        # Removing phenotype-related redundancies
        evidences = PanelApp_evidence_generator.removing_redundancies(evidences)

        return evidences

    @staticmethod
    def removing_redundancies(evidences):

        # Removing redudant evidences
        parsed_data = []

        json_data = evidences.apply(lambda x: json.loads(x))
        print("tipo",json_data)
        ...
        for (target, disease, panel_id), group in panelapp_df.groupby(['target','disease','panel_id']):
            ...
            # Update evidence:
            data['disease']['source_name'] = group.phenotype.unique().tolist()
            # Save evidence:
...
def main():
    ...
    # Writing evidence strings into a json file and removing evidences
