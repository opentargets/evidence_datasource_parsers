...
class progenyEvidenceGenerator:
    ...
    def __init__(self):
        ...
        self.spark = SparkSession.builder.appName("progeny").getOrCreate()

        # Initialize source table
        self.dataframe = None

    def generateEvidenceFromSource(
        self, inputFile, diseaseMapping, pathwayMapping, skipMapping
    ):
        """
        ...
        self.dataframe = (
            ...
            .csv(inputFile)
        ).withColumn("literature", F.array(F.lit(self.PMID)))
        ...
        if not skipMapping:
            try:
                ...
                logging.error(
                    f"An error occurred while importing disease mappings: \n{e}."
                )
        else:
        ...
        evidences = (
            self.dataframe.rdd.map(
                progenyEvidenceGenerator.parseEvidenceString
    ...
    def cancer2EFO(self, diseaseMapping):
        diseaseMappingsFile = (
            self.spark.read.csv(
                "resources/cancer2EFO_mappings.tsv", sep=r"\t", header=True
            )
        ...
        self.dataframe = self.dataframe.join(
            diseaseMappingsFile, on="Cancer_type", how="left"
        )

        return self.dataframe

    def pathway2Reactome(self, pathwayMapping):
        pathwayMappingsFile = self.spark.read.csv(
            "resources/pathway2Reactome_mappings.tsv", sep=r"\t", header=True
        ).withColumnRenamed("pathway", "Pathway")

        self.dataframe = (
            self.dataframe.join(pathwayMappingsFile, on="Pathway", how="inner")
    ...
    def parseEvidenceString(row):
        try:
            evidence = {
                ...
                "pathways": [{"id": row["reactomeId"], "name": row["description"]}],
...
def main(inputFile, diseaseMapping, pathwayMapping, outputFile, skipMapping):
    ...
    evidences = evidenceBuilder.generateEvidenceFromSource(
        inputFile, diseaseMapping, pathwayMapping, skipMapping
    )
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="This script generates evidences for the PROGENy data source."
    )

    parser.add_argument(
        "-i", "--inputFile", required=True, type=str, help="Input source .tsv file."
    )
    parser.add_argument(
        "-d",
        "--diseaseMapping",
    ...
    parser.add_argument(
        ...
        required=True,
        type=str,
        help="Input look-up table containing the pathway mappings to a respective target and ID in Reactome.",
    )
    parser.add_argument(
        "-o",
        "--outputFile",
