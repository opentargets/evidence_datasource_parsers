...
def main(
    ...
    burden_evidence_sets = [
        ...
        process_az_gene_burden(
            spark, az_binary_data, az_quant_data, az_genes_links, az_phenotypes_links
        ).persist(),
...
def process_finngen_gene_burden(
    ...
    manifest = spark.read.json(finngen_manifest).selectExpr(
        "phenocode as PHENO", "phenostring as diseaseFromSource"
    )
    ...
    finngen_df = (
        ...
        .withColumn("resourceScore", 10 ** -f.col("LOG10P").cast("float"))
        .withColumn(
        ...
        .select(
            ...
            f.array(f.lit("36653562")).alias("literature"),
            f.col("resourceScore"),
            ...
            f.lit(
                "Burden test carried out with LoF variants with MAF smaller than 1%."
    ...
    return finngen_df.filter(
        f.col("resourceScore") <= statistical_significance
    ).distinct()
...
def process_gene_burden_curation(spark: SparkSession, curated_data: str) -> DataFrame:
    ...
    logging.info(
        f"Total number of imported manual gene_burden evidence: {manual_df.count()}"
    )
    ...
    manual_df = (
        ...
        .withColumn(
            ...
            f.when(f.col("oddsRatio").isNotNull(), f.col("ConfidenceIntervalLower")),
        )
        .withColumn(
            "oddsRatioConfidenceIntervalUpper",
            f.when(f.col("oddsRatio").isNotNull(), f.col("ConfidenceIntervalUpper")),
        )
        .withColumn(
        ...
        .withColumn(
            "allelicRequirements",
            f.when(
                f.col("allelicRequirements").isNotNull(),
                f.array(f.col("allelicRequirements")),
            ),
        )
...
def read_gene_burden_curation(spark: SparkSession, curated_data: str) -> DataFrame:
    ...
    return spark.read.csv(
        SparkFiles.get(curated_data.split("/")[-1]),
        sep="\t",
        header=True,
        schema=schema,
    )
