...
def main(chembl_evidence: str, predictions: str, chembl_indications: str, output_file: str) -> None:
    """
    ...
    Args:
        ...
        chembl_indications (str): Input gzipped JSON with curation of drug/indication pairs maintained by ChEMBL.
    ...
    logging.info(f'ChEMBL drug/indication pairs JSON file: {chembl_indications}')
    ...
    drug_approvals_df = (
        spark.read.json(chembl_indications)
        .filter(f.col("max_phase_for_ind") == 4)
    ...
    # Remove T/D evidence from Phase IV trials that have not demonstrated efficacy
    filtered_chembl_df = remove_unvalidated_target_disease(enriched_chembl_df, drug_approvals_df)

    logging.info('Evidence strings have been processed. Saving...')
    write_evidence_strings(filtered_chembl_df, output_file)
...
def get_parser():
    ...
    parser.add_argument(
        '--chembl_indications',
        help='Input gzipped JSON with approved and investigational indications for drug molecules.',
        type=str,
        required=True,
    )
    parser.add_argument(
