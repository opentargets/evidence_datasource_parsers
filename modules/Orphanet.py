...
def parse_orphanet_xml(orphanet_file: str, spark_instance) -> DataFrame:
    ...
    logging.info(
        f"There are {root.find('DisorderList').get('count')} disease in the Orphanet xml file."
    )
    ...
    for disorder in root.find("DisorderList").findall("Disorder"):
        ...
        for association in disorder.find("DisorderGeneAssociationList"):
            ...
            evidence["associationType"] = association.find(
                "DisorderGeneAssociationType/Name"
            ).text
            evidence["confidence"] = association.find(
            ...
            evidence["targetFromSourceId"] = (
                ensembl_gene_id[0] if len(ensembl_gene_id) > 0 else None
            )
